...
import commands.public_commands as public_commands   # noqa: E402
...
async def test_command_list_handler_empty(mock_message):
    ...
    with patch.object(
        public_commands, "get_whitelist_by_chat", mock_get_whitelist_by_chat
    ), patch.object(
        public_commands, "get_chat_status", mock_get_chat_status
    ), patch.object(
        public_commands, "dp", {"telethon_helper": AsyncMock()}
    ):
...
@pytest.mark.asyncio
async def test_command_list_handler_with_users(
    mock_message, mock_telethon_helper
):
...
@pytest.mark.asyncio
async def test_command_add_user_handler(
    ...
    with patch.object(
        ...
        public_commands, "add_user_to_whitelist", mock_add_user_to_whitelist
    ):

        await public_commands.command_add_user_handler(
            mock_message_admin, mock_command
        )
...
@pytest.mark.asyncio
async def test_command_add_user_handler_no_args(
    mock_message_admin, mock_command
) -> None:
...
@pytest.mark.asyncio
async def test_command_remove_user_handler(
    mock_message_admin, mock_command
) -> None:
    mock_command.args = "@username"
    ...
    with patch.object(
        ...
        mock_remove_user_from_whitelist
    ), patch.object(
        public_commands, "get_chat_status", mock_get_chat_status
    ):

        await public_commands.command_remove_user_handler(
            mock_message_admin, mock_command
        )
...
@pytest.mark.asyncio
async def test_command_remove_user_handler_user_not_found(
    mock_message_admin, mock_command
) -> None:
    mock_command.args = "@username"
...
async def test_command_add_all_members_handler(
    ...
    with patch.object(
        ...
        await public_commands.command_add_all_members_handler(
            mock_message_admin, mock_command
        )
...
async def test_command_unpause_handler_not_admin(
    mock_message_user, mock_command, mock_telethon_helper
) -> None:
    mock_update_pause_status = AsyncMock()
...
async def test_command_remove_all_members_handler(
    ...
    with patch.object(
        ...
        await public_commands.command_remove_all_members_handler(
            mock_message_admin, mock_command
        )
...
async def test_handle_new_chat_member_not_paused_not_in_whitelist(
    ...
    with patch.object(
        ...
        public_commands, "is_user_in_whitelist", mock_is_user_in_whitelist
    ), patch.object(
        public_commands, "dp", {"telethon_helper": mock_telethon_helper}
    ):
